...
const MAX_EVENTS = Math.max(50, Number(process.env.DTUNES_MAX_EVENTS || 5000));
const MAX_RAW_METADATA_BYTES = 2048;
...
// Client payloads used to embed (and persist) their entire song object as
// raw metadata, including streaming URLs and every image size. Keep only the
// small identity fields so the data file cannot balloon per event.
function compactRawMetadata(input) {
  if (!input || typeof input !== 'object') return {};
  const picked = {};
  for (const key of ['saavn_id', 'id', 'title', 'name', 'primary_artists', 'artist', 'album', 'language', 'year', 'image_url', 'duration_seconds']) {
    const value = input[key];
    if (value !== undefined && value !== null && String(value).length <= 256) picked[key] = value;
  }
  try {
    if (JSON.stringify(picked).length > MAX_RAW_METADATA_BYTES) return {};
  } catch {
    return {};
  }
  return picked;
...
function pruneEvents(data) {
  if (data.user_events.length > MAX_EVENTS) {
    data.user_events = data.user_events.slice(-MAX_EVENTS);
  }
}
...
function toDbSong(input = {}) {
  return {
    ...
    raw_metadata_json: compactRawMetadata(input.raw_metadata_json || input.raw || input),
  };
}
...
function recordEvent({ userId, song, songSaavnId, eventType, playDurationSeconds, songDurationSeconds, context = {} }) {
  ...
  // Rebuild the taste profile from the same in-memory data and persist
  // everything with a single file write.
  rebuildUserTasteProfile(event.user_id, data);
  pruneEvents(data);
  save(data);
  return event;
}

function rebuildUserTasteProfile(userId, existingData = null) {
  const data = existingData || load();
...
module.exports = { upsertSong, recordEvent, rebuildUserTasteProfile, getTasteProfile, getRecentEvents, getUserSongScores, getLocalCandidates, load, MAX_EVENTS, compactRawMetadata };

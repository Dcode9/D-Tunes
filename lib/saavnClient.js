...
// Multiple community mirrors of the same JioSaavn API contract; fetchJson
// fails over to the next endpoint when one is rate-limited or down.
const API_ENDPOINTS = [
...
async function fetchJson(path, { endpoints = API_ENDPOINTS, timeout = 4000 } = {}) {
  let lastError;
...
async function getTrending(limit = 25) {
  ...
  if (allSongs.length < limit) {
    ...
    const fallbackQueries = [`Top Bollywood Hits ${year}`, 'Global Top Hits', `Viral Songs ${year}`, 'Trending Pop'];
